...
int Key_Schedule(unsigned char *Seedkey, int KeyLen, unsigned char Direction, unsigned char *Subkey,int rounds)
{
    ...
    MatGf2 mat = make_special_rotate(32);
    MatGf2 mat_back = make_special_rotate_back(32);
    for (i = 0; i < rounds; i++)
    {
...
int _swan_whitebox_content_assemble(swan_whitebox_helper *swh, swan_whitebox_content *swc)
{
    ...
    B_ptr = swc->B;
    P_ptr = swc->P+1;
    MatGf2 rotate_back = make_special_rotate_back(piece_count * SWAN_PIECE_BIT);
            ...
            for (k = 0; k < piece_count; k++)
            {
                ...
                for (t = 0; t < 4; t++)
                {
                    ...
                    if (t != k)
                    {
                        ...
                        switchxor = switchxor ^ randomNum[t][t];
                
                    }
                }
                
                C_ptr->sub_affine[k].vector_translation = GenMatAddU8(C_ptr->sub_affine[k].vector_translation, ApplyMatToU8(C_ptr->sub_affine[k].linear_map, switchxor));
                ...
                for (i = 0; i < piece_range; i++)
                {
                    ...
                    *((uint32_t *)yc) = MATtoU32(rotate_back, *((uint32_t *)yc));

                    if (k != 3)
                    {
                        for (j = 0; j < 4; j++)
                        {
                            yc[j] = yc[j] ^ randomNum[k][j];
                        }
                    }
                    else
                    {
                        yc[0] = yc[0] ^ randomNum[1][0] ^ randomNum[2][0];
                        yc[1] = yc[1] ^ randomNum[0][1] ^ randomNum[2][1];
                        yc[2] = yc[2] ^ randomNum[0][2] ^ randomNum[1][2];
                        yc[3] = yc[3] ^ randomNum[0][3] ^ randomNum[1][3] ^ randomNum[2][3] ^ randomNum[3][3];
                    }
    ...
    // memcpy(swc->C, swh->C, swh->rounds  * sizeof(CombinedAffine));
    // memcpy(swc->B, swh->B, swh->rounds * sizeof(CombinedAffine));
    // memcpy(swc->P, swh->P, (swh->rounds + 2)  * sizeof(CombinedAffine));
